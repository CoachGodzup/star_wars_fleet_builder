...
export const TableFleet: React.FC<TableFleetProps> = ({ assignments }) => {
    return <Table>
        {assignments.map((assignment, index) => <Table.Tr key={index + assignment.starship.MGLT}>
        <Table.Td>
            <Group gap="sm">
                <Avatar size={40} color={'initials'} radius={40} name={assignment.starship.name}></Avatar>
                <div>
                <Text fz="sm" fw={500}>
        ...
        <Table.Td>
            {assignment.general ? 
            <Group gap="sm">
                <Avatar size={40} radius={40} name={assignment.general.name} color={'initials'}></Avatar>
                <div>
                    <Text fz="sm" fw={500}>
